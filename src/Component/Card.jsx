...
export const Card = ({pic,title,desc}) => {
  return (
    <>
        <div className='max-w-[23rem] h-[30rem]  rounded-lg shadow-2xl bg-white'>
        <div className='h-[15rem] w-full rounded-lg'>
            <img src={pic} alt="Content Writing" className='h-full w-full rounded-lg object-fill' />
        </div>
        <article className='px-3 py-5 flex flex-col gap-5'>
        <h1 className='text-xl font-bold'>{title}</h1>
           <p className='text-[1rem] text-wrap'>
                 {desc}
              </p>
 ...
 export const BlogCard =({mt,title,img,date,read,author})=>{
    return(
        <>
        <div className={`h-[31rem]  bg-black relative rounded-2xl group overflow-hidden ${mt}`}>
          <img src={img} alt="pic"  className='h-full w-full  object-cover rounded-2xl' />
          <div className='absolute  h-full w-full bg-black/80 top-0 left-0 right-0 rounded-2xl'></div>
          <div className='absolute -bottom-[30rem] group-hover:bottom-0 transition-all delay-100 group-hover:transition-all left-0 right-0 
           bg-black/90 rounded-br-2xl rounded-bl-2xl
            text-white h-[10rem] px-3 py-4 flex flex-col gap-1 justify-center'>
              <p className='flex items-center gap-2'> 
                <span className='font-bold text-lg'><CiCalendarDate /></span> <span className='font-thin text-[1rem]  '>{date}</span></p>
